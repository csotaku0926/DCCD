...
class Encoder(nn.Module):
    ...
    def forward(self, x: torch.FloatTensor, x2: torch.FloatTensor, mask: torch.ByteTensor) -> torch.FloatTensor:
        """Pass the input (and mask) through each layer in turn."""
        x = self.layers[0](x, x2, mask)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x: torch.FloatTensor, x2: torch.FloatTensor, mask: torch.ByteTensor=None) -> torch.FloatTensor:
        return self.encoder(x, x2, mask)
